...
# **Answer:**
# The five most frequently appearing words are movi, film, one and time. That makes sense because the reviews are about movie reviews.
...
# **Answer:**
# I could be happen that some informations could be lost but it is helpful because it make it possible to get consistency in the data.
...
# XGBoost needs lees training. The earlier model maybe need some more epoch. Maybe hyperparameter tunning could increase the performance. 
...
test_X, test_X_len = convert_and_pad_data(word_dict, input_data_words)
test_data = pd.concat([pd.DataFrame(test_X_len), pd.DataFrame(test_X)], axis=1)
...
# **Answer:**
# - Ohh, nice one --> Your review was POSITIVE!
# - Hmm, what was that? --> Your review was NEGATIVE!
